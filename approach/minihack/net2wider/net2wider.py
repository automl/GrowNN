...
from utils.minihack.feature_extractor import Net2WiderFeatureExtractor
...
def black_box_ppo_configure(config: Configuration):
    def black_box_ppo_execute(result_processor: ResultProcessor):
        ...
        feature_extractor_width_step = config.non_hyperparameters.feature_extractor_width_step
        seed = config["seed"]
        ...
        if feature_extractor_width_step > 1:  # If we want to incerase the width of the feature extractor
            # Rebuild feature Extractor of previous budget
            for _ in range(1, feature_extractor_width_step - 1):
                model.policy.features_extractor.increase_width()
                model.policy.to("cuda" if torch.cuda.is_available() else "cpu")
                model.policy.optimizer = torch.optim.Adam(model.policy.parameters(), lr=learning_rate)

            # Load the model of the previous budget
            final_load_path = get_model_save_path_minihack(config.non_hyperparameters.model_save_path, config, feature_extractor_width_step - 1, seed)
            model.set_parameters(os.path.join(final_load_path, "model.zip"), exact_match=False)

            # Increase the width of the feature extractor
            model.policy.features_extractor.increase_width()
            model.policy.to("cuda" if torch.cuda.is_available() else "cpu")
            model.policy.optimizer = torch.optim.Adam(model.policy.parameters(), lr=learning_rate)
        ...
        final_save_path = get_model_save_path_minihack(config.non_hyperparameters.model_save_path, config, feature_extractor_width_step, seed)
        ...
        if not debug_mode:
            callback_wrapper.process_results(
                ...
                budget=feature_extractor_width_step,
